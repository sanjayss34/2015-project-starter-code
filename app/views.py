...
def add_treasure():
    ...
    for c in coordinate_objs:
        ...
        treasures.append(models.treasurepoint(latitude=c['latitude'], longitude=c['longitude'], notes=str(c['notes'])))
        db.session.add(models.treasurepoint(latitude=c['latitude'], longitude=c['longitude'], notes=str(c['notes'])))
        db.session.commit()
